...
- **A2A Protocol v1.0 Agent ID Cards** — HTTP Agent Card + MCP `x402.agent_card` (back-compat alias: `get_agent_card`) / `x402://agent-card` with per-`agent_id` quota isolation
...
| `mailrail.inbound` | Ingest or route an inbound message to the MailRail Postmaster agent with mention defusing and inbox ledger recording |
...
**scout** discovers cheap upstream x402 services → **warden** enforces `ledger/policy.json` spend caps → **treasurer** `pay_and_fetch`es and records cost basis to `ledger/spend.jsonl` → **archivist** composes a research report priced at `cost × SWARM_MARKUP` → **sovereign** (profit optimizer) reprices the composite to hit a target LTV:CAC (`SWARM_TARGET_LTV_CAC`, default 3.0), enforces a margin floor, and scores which upstream sources are actually profitable → **merchant** lists it via `build_seller_requirements`; `settle_composite_sale` records realized revenue. Portfolio economics (swarm spend, composite sales, LTV:CAC, per-source profit) surface via `swarm_revenue_report` / `GET /swarm/revenue` — `total_revenue_usdc` is composites only; first-party SKUs are under `storefront`. Every phase streams to the dashboard's Swarm Activity panel over SSE. Run via the `run_swarm_research` MCP tool (needs `EVM_PRIVATE_KEY` + `X402_PAY_TO_ADDRESS`).
...
2. **MCP tool:** `x402.agent_card` (back-compat alias: `get_agent_card`; optional `target_id` for a skill; implemented by server handler `get_agent_card`)
